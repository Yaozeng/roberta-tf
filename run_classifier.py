...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if ex_index < 5:
    ...
    tf.logging.info("tokens_origin: %s" % tokenizer.decode(input_ids))
    tf.logging.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
    tf.logging.info("input_mask: %s" % " ".join([str(x) for x in attention_mask]))
    tf.logging.info("segment_ids: %s" % " ".join([str(x) for x in token_type_ids]))
