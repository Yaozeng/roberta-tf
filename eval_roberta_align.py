...
with graph.as_default():
    ...
    model = modeling2.BertModel(
        ...
        align_mask=align_mask_placehold,
        token_type_ids=None,
...
with  tf.Session(graph=graph) as sess:
    model_file=tf.train.latest_checkpoint("./output/roberta_align/")
    ...
    for path in paths:
        ...
        for j in range(1, nrows):
            ...
            for i in range(len(samples)):
                ...
                align_mask = [[0] * text_a_len + [1] * text_b_len + [0] * padding_length] * text_a_len + [
                    [1] * text_a_len + [0] * (text_b_len + padding_length)] * text_b_len + [
                                 [0] * len(input_ids)] * padding_length
                ...
                if all_align_masks is None:
                    all_align_masks = np.array(align_mask, dtype=np.int).reshape([1,64, 64])
                else:
                    all_align_masks = np.append(all_align_masks, np.array(align_mask, dtype=np.int).reshape([1,64, 64]), axis=0)
            ...
            prob=sess.run(probabilities,feed_dict={input_ids_placehold:all_input_ids,input_mask_placehold:all_input_masks,align_mask_placehold:all_align_masks})
            prob=np.array(prob)
            #print(prob.shape)
